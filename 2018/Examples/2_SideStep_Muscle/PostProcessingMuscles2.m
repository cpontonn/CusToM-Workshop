...
%% Music Method Results
cd .. 
cd('SideStep_Muscle_Music');
...
% of the desired muscles
R_ActivationsMusic=Activations(num_Rm,:);
L_ActivationsMusic=Activations(num_Lm,:);
...
for ii=1:6
    ...
    hold on
    plot(R_ActivationsMusic(ii,:)*100,'b-','LineWidth',2)
    xlim([0 Nb_frames])
    ylim([0 100])
    xlabel('Frames')
...
legend('Optimization','Music')

figure
for ii=1:6
    subplot(2,3,ii)
    plot(L_ActivationsOpti(ii,:)*100,'r-','LineWidth',2)
    hold on
    plot(L_ActivationsMusic(ii,:)*100,'b-','LineWidth',2)
    xlim([0 Nb_frames])
    ylim([0 100])
    xlabel('Frames')
    ylabel('Muscle Activation (%)')
